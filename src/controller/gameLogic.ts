...
export class GameLogic {
  ...
  public chow(userID: string): boolean {
    const nextUser: string =
      this.userIDs[(this.currentPlayerIndex + 1) % this.userIDs.length];
    if (userID == nextUser) {
      if (this.trivia()) {
  ...
  public discard(tileID: string) {
    ...
    let tileIDsInHand: Set<string> = new Set<string>();
    currPlayersHand.forEach((tile) => {
      tileIDsInHand.add(tile.id);
    });

    if (tileIDsInHand.has(tileID)) {
      this.currentDiscarded = currPlayersHand.find(
        (tile) => tile.id === tileID
      ) as ITile; // get tile from dataBase
  ...
  private checkTiles(checkType: String): boolean {
    ...
    if (checkType == "chow") {
      ...
      let playersMatchingType: Set<ITile> = new Set<ITile>();
      currPlayersHand.forEach((tile) => {
        if (tile.tileType == this.currentDiscarded.tileType) {
          playersMatchingType.add(tile);
        }
      });
      let playersTileNums: Set<string> = new Set<string>();
      playersMatchingType.forEach((tile) => {
        playersTileNums.add(tile.tileNum);
      });

      if (
        String(Number(this.currentDiscarded.tileNum) + 1) in playersTileNums &&
        String(Number(this.currentDiscarded.tileNum) + 2) in playersTileNums
      ) {
        return true;
      } else if (
        String(Number(this.currentDiscarded.tileNum) - 1) in playersTileNums &&
        ...
        return true;
      } else if (String(Number(this.currentDiscarded.tileNum) - 1) in playersTileNums && String(Number(this.currentDiscarded.tileNum) + 1) in playersTileNums) {
        return true;
      }
      return false;
    } else if (checkType == "pung") {
      // checks to see that the
      let playersMatching: Array<ITile> = new Array<ITile>();
      currPlayersHand.forEach((tile) => {
        if (
          tile.tileType === this.currentDiscarded.tileType &&
          tile.tileNum === this.currentDiscarded.tileNum
        ) {
...
function main() {
  ...
  let idNum: number = 0;
  connectMongo(() => {
    connectedToMongo() ? void 0 : void 0;
  });
  for (let tileType = 0; tileType < 10; tileType++) {
    if (tileType < 3) {
      for (let tileNum = 0; tileNum < 10; tileNum++) {
        for (let index = 0; index < 4; index++) {
          createTile({
            tileType: String(tileType),
            tileNum: String(tileNum),
            id: String(idNum),
            visibility: House.NONE,
            hand: House.NONE,
          });
          idNum++;
        }
